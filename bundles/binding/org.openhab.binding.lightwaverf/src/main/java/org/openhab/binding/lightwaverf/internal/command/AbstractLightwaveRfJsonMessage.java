...
public abstract class AbstractLightwaveRfJsonMessage implements LightwaveRFCommand {
	...
	public AbstractLightwaveRfJsonMessage(String message) {
	...
	protected String getStringFromText(Pattern regExp, String message) {
		Matcher matcher = regExp.matcher(message);
		matcher.matches();
		return matcher.group(1);
	}

	protected Date getDateFromText(Pattern regExp, String message) {
		long timeInSeconds = getLongFromText(regExp, message);
		long timeInMillis = timeInSeconds * 1000;
		return new Date(timeInMillis);
	}

	protected int getIntFromText(Pattern regExp, String message) {
		return Integer.valueOf(getStringFromText(regExp, message));
	}

	protected double getDoubleFromText(Pattern regExp, String message) {
		return Double.valueOf(getStringFromText(regExp, message));
	}
	
	protected long getLongFromText(Pattern regExp, String message) {
