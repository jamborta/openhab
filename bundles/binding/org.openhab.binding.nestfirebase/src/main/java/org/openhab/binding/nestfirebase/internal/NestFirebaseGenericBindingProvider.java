 ...
 * @since 1.8.0
 */
public class NestFirebaseGenericBindingProvider extends AbstractGenericBindingProvider implements NestFirebaseBindingProvider {
	private static final Logger logger = LoggerFactory.getLogger(NestFirebaseGenericBindingProvider.class);
	...
	public void processBindingConfiguration(String context, Item item, String bindingConfig) throws BindingConfigParseException {
		...
		try{
			...
			NestFirebaseBindingConfig config = new NestFirebaseBindingConfig(id, type);
	...
	public List<String> getItemNameFromNestId(String nestId) {
		...
		for (String itemName : bindingConfigs.keySet()) {
			NestFirebaseBindingConfig itemConfig = (NestFirebaseBindingConfig) bindingConfigs.get(itemName);
	...
	public NestType getTypeForItemName(String itemName) {
		NestFirebaseBindingConfig config = (NestFirebaseBindingConfig) bindingConfigs.get(itemName);
		if(config != null){
	 ...
	 * @since 1.7.0
	 */
	class NestFirebaseBindingConfig implements BindingConfig {
		
		private final String id;
		private final NestType type;

		public NestFirebaseBindingConfig(String id, NestType type) {
			this.id = id;
			this.type = type;
		}
